...
**Default mode:** Proceed with the overwrite without waiting.

**Buddy mode:** Wait for confirmation before continuing.
...
**Buddy mode:** Wait for confirmation or corrections. Do not write any file until the user approves.
...
3. If `feature-detail` is not already in the `artifacts` list, append it.

Use the Edit tool. Example — before:
```yaml
  - title: my-work-item
    hierarchyLevel: Product
    path: docs/product/features/20260504-my-feature/
```
After:
```yaml
  - title: my-work-item
...
If `artifacts` already exists, append `feature-detail` to the list. Never duplicate an entry already present.
